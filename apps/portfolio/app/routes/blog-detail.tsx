...
export async function loader({ params }: { params: { slug: string } }) {
  ...
  return {
    ...
    prev: prev ? { title: prev.data.title, link: `/blog/${prev.slug}` } : null,
    next: next ? { title: next.data.title, link: `/blog/${next.slug}` } : null,
  }
}
...
export default function BlogDetail({ loaderData }: { loaderData: LoaderData }) {
  const {
    slug,
    title,
    publishedAt,
